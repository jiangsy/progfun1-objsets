...
abstract class TweetSet {
  ...
  /**
    ...
    * and be implemented in the subclasses?
    */
  def union(that: TweetSet): TweetSet = filterAcc(x=>true,that.filterAcc(x=>true,new Empty()))
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  // if (p(elem)) this.right.filterAcc(p,this.left.filterAcc(p,acc.incl(elem)))
  ...
  def mostRetweeted = {
    ...
    if (elem.retweets >lefttweet.retweets)
      if (elem.retweets > righttweet.retweets) elem
      else righttweet
